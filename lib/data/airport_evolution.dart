...
class AirportEvolution {
  ...
  /// 250 на первом, ~7150 на последнем, около 40 000 за всю ветку.
  /// Для сравнения: все 150 головоломок дают примерно 15 000 монет,
  /// поэтому одними уровнями ветку не закрыть - нужен и рейс дня,
  /// и ежедневный доход, и время.
  static int costFor(int level) {
    if (level < 1) return 0;
    if (level > maxLevel) return 0;
    final double raw = 250 * math.pow(1.15, level - 1).toDouble();
    return (raw / 10).round() * 10;
  }

  /// Пассивный доход раз в сутки. На максимуме - около 470 монет,
  /// это примерно четыре пройденные головоломки: ощутимо, но
  /// не отменяет необходимость играть.
